...
interface Categoria {
  ...
  tipoBoton?: string; urlPagina?: string | null
...
export default function CategoriaGrid({ categorias }: { categorias: Categoria[] }) {
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      {categorias.map(cat => (
        <Link key={cat.id} href={cat.tipoBoton === 'pagina' && cat.urlPagina ? cat.urlPagina : `/categoria/${cat.slug}`}
          ...
          {cat.tipoBoton === 'pagina' ? (
            <p className="text-xs text-rojo-principal mt-0.5 font-medium">Ir a la página →</p>
          ) : cat._count && (
